...
# Define the path for the data file (Excel) and log file
...
class SalesCRM:
    ...
    def display_company_name(self):
        company_name_label = tk.Label(self.root, text="AMAR GROUP", font=("Helvetica", 24, "bold"))
    ...
    def add_po_popup(self):
        ...
        def add_po():
            try:
                ...
                self.data[lead_index][24] = po_number  # PO Number
                self.data[lead_index][25] = po_date  # PO Date
                self.data[lead_index][26] = po_value  # PO Value
                self.data[lead_index][27] = products_details  # Products Details
                self.data[lead_index][28] = delivery_date  # Delivery Date

                save_data(self.data)
                log_change(f"Added PO for Lead Index {lead_index + 1}")
                messagebox.showinfo("Success", f"PO for Lead Index {lead_index + 1} added successfully.")
                self.add_po_window.destroy()

            except ValueError:
                messagebox.showerror("Invalid Input", "Please enter a valid Lead Index and PO Details.")
            except IndexError as e:
                messagebox.showerror("Invalid Lead Index", str(e))
